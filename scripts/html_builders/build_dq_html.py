...
rtcov=json.load(open(f'{OUT}/rt_social_rtcov.json'))  # RT-covariate re-run of surviving social clusters
rtcov_rows=''.join(f"<tr><td>{c['label']}</td><td>{c['p0']}</td><td><b>{c['p1']}</b></td><td>{c['n']}</td></tr>" for c in rtcov)

def sig(p): return '<span class="yes">significant</span>' if p<0.05 else '<span class="no">n.s.</span>'
def pf(p):  # never show a bare "0.0" for a permutation/t-test p — floor it
    return '&lt;0.001' if float(p)<0.001 else f'{float(p):g}'
...
<tr><td>Social d′</td><td>{dp['social']['mean']:+}</td><td>{dp['social']['t_vs0']}</td><td>{pf(dp['social']['p_vs0'])}</td><td>{'<span class=yes>above chance</span>' if dp['social']['p_vs0']<0.05 else '<span class=no>at chance</span>'}</td></tr>
<tr><td>Monetary d′</td><td>{dp['monetary']['mean']:+}</td><td>{dp['monetary']['t_vs0']}</td><td>{pf(dp['monetary']['p_vs0'])}</td><td>{'<span class=yes>above chance</span>' if dp['monetary']['p_vs0']<0.05 else '<span class=no>at chance</span>'}</td></tr>
</tbody></table>
<h3 style="font-size:14px;color:var(--mut);margin-top:16px">Correlated with each other?</h3>
<div class="callout"><b>No.</b> Pearson r = <b>{dp['correlation']['r']}</b>, p = {dp['correlation']['p']} (n={dp['correlation']['n']}). The two memory scores are essentially independent — a subject good at social memory isn't necessarily good at monetary memory. This matters for the tract results: <b>only the social measures track the VTA→HPC tract</b> (monetary shows no surviving cluster), and because social and monetary d′ are uncorrelated, that specificity reflects genuinely distinct abilities rather than two views of one signal.</div>
...
<div class="mut" style="font-size:12.5px">Both tests agree the two differ (paired t p={dp['paired_diff']['p']}, Wilcoxon p={dp['paired_diff']['wilcoxon_p']}): <b>monetary</b> discrimination is higher on average, while <b>social</b> d′ sits just above chance (p={dp['social']['p_vs0']}). They remain uncorrelated at the subject level — and, notably, it is the weaker social measure (and social positivity bias) that tracks the VTA→HPC tract, not the stronger monetary one.</div>
</div>
...
<div class="callout">Our independent recomputation of d′ from the raw trial files reproduces the analysis values closely — monetary near-perfectly (r = 0.99) and social strongly (r = 0.81; the social recompute is noisier because chosen/not-chosen item matching in the raw logs is more ambiguous):
...
<div class="callout" style="border-left-color:var(--yes)"><b>The tract findings are NOT an RT artifact.</b> On this sample neither social d′ (r={rt['corr_with_dprime'][0]['r']}, p={rt['corr_with_dprime'][0]['p']}) nor monetary d′ (r={rt['corr_with_dprime'][1]['r']}, p={rt['corr_with_dprime'][1]['p']}) is significantly related to recall speed, so response caution is not even a candidate confound for accuracy. To be thorough we still re-ran every surviving <b>social</b> FWE cluster (all findings are social — see the Results Explorer) with recall-RT added as a covariate. <b>All survived essentially unchanged:</b>
<table style="margin-top:8px"><thead><tr><th>Social cluster (mean of significant nodes)</th><th>p, covariate-adjusted</th><th>p, + recall-RT</th><th>n</th></tr></thead><tbody>{rtcov_rows}</tbody></table>
The tract-microstructure signal is independent of response speed.</div>
</div>
