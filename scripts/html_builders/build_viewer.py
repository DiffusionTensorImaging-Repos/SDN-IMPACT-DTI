#!/usr/bin/env python3
"""
Build results_html/explorer_viewer.html — a reusable, fully data-agnostic viewer for
...
OUT = Path("/Users/dannyzweben/Desktop/SDN/DTI/SDN-IMPACT-DTI/results_html")
src = (OUT / "results_explorer.html").read_text()
css = re.search(r"<style>.*?</style>", src, re.S).group(0)  # reuse the exact dark theme
...
DOC = """<div class="doc">
<b>Your permutation script already produced almost exactly what this needs.</b> For each analysis (one outcome × tract × metric), <code>permutation_one.R</code> writes <code>&lt;tract&gt;__&lt;metric&gt;__&lt;outcome&gt;_nodewise.csv</code>, one row per node: <code>Node, Estimate, t_value, p_value, df, n</code>. The explorer wants those node rows <b>stacked into one CSV with three label columns added</b> (outcome, tract, metric) so it knows which analysis each row belongs to.
<br><br><b>Fastest way — run the stacker on your results folder.</b> It reads every <code>*_nodewise.csv</code>, adds the labels from the filename and the N / covariates / extent-threshold / cluster-p from the matching <code>_summary.csv</code> and <code>_clusters.csv</code>, and writes one CSV you drop above:
<div class="script" style="margin:6px 0">python scripts/stack_permutation_results.py  &lt;permutation_results_dir&gt;  my_results.csv  [--hemi]</div>
<code>--hemi</code> adds a hemisphere (L/R) column parsed from the tract name; leave it off if your tracts aren't lateralized.
<br><br><b>Or build it by hand.</b> One row per node. Column names are case-insensitive, and the R script's own names are accepted as-is (<code>Node</code>, <code>t_value</code>, <code>p_value</code>, <code>N_subjects</code>, <code>Covariates</code>, <code>ExtentThresholdNodes</code>, <code>ClusterPValue</code>, <code>PassExtentThreshold</code>):
...
{BODY}
{JS}</body></html>'''
(OUT / "explorer_viewer.html").write_text(html)
print(f"wrote explorer_viewer.html ({len(html)} bytes)")
