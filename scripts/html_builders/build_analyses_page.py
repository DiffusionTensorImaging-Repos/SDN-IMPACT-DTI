...
<div class="wrap">
  ...
  <p class="small">"Positive / negative" is how she <b>remembered</b> each item (every response is tagged positive, negative, or neutral). TrueMem = an item she'd chosen at encoding and correctly recognized; FalseMem = a foil she falsely remembered. Total<sub>pos</sub> and Total<sub>neg</sub> are her total positive and negative responses on the test, so each score is a positive-minus-negative skew in what she remembers.</p>
  {s_bias}
  ...
  <table class="dtable"><thead><tr><th>Component</th><th>Social</th><th>Monetary</th><th>differ?</th></tr></thead><tbody>
    <tr><td>Hit rate</td><td>0.560</td><td>0.539</td><td>no (p=.40)</td></tr>
    <tr><td>False-alarm rate</td><td>0.527</td><td>0.460</td><td class="sig">yes (p=.008)</td></tr>
    <tr><td>d′ (accuracy)</td><td>0.10 ± .29</td><td>0.22 ± .33</td><td>no (p=.07)</td></tr>
    <tr><td>HitRateBias (correct-memory positivity)</td><td>+0.12 ± .19</td><td>+0.12 ± .16</td><td>no (p=.92)</td></tr>
    <tr><td>FABias (false-memory positivity)</td><td>+0.07 ± .19</td><td>+0.08 ± .16</td><td>no (p=.82)</td></tr>
  </tbody></table>
  <p class="small">Paired t-tests, n = 52. The only difference between domains is the <b>false-alarm rate</b>: the social task produces more false alarms. Hit rate, d′, and both positivity biases are equivalent, so the lower social d′ is more false positives, not weaker memory, and the social signal lives specifically in false memories.</p>
  {s_breakdown}
